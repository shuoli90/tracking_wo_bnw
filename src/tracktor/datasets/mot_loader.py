...
class MOT17Sequence(Dataset):
    ...
    def __init__(self, seq_name=None, dets='', vis_threshold=0.0,
        ...
        if seq_name is not None:
            ...
            self.label = np.genfromtxt(f"test_results/{seq_name}-FRCNN_data.csv", delimiter=',')
        else:
            self.data = []
            self.no_gt = True
    ...
    def __getitem__(self, idx):
        ...
        # sample['vis'] = data['vis']
        # sample['']
        # sample['label'] = self.label[self.label[:,0]==self.frame,]
        label = self.label[self.label[:,0]==self.frame,]
        sample['frame_index'] = label[:,0]
        sample['kalman_mean'] = label[:, 1:5]
        sample['kalman_variance'] = label[:, 5:9]
        sample['ground_truth'] = label[:, 9:13]
        sample['predicted_visibility'] = label[:, 13]
        sample['is_occluded'] = label[:, 14]
        sample['track_id'] = label[:, 15]
        sample['object_id'] = label[:, 16] 

        self.frame += 1

        # breakpoint()

        # sample = {'img':img, 'gt':data['gt']}
        return sample
...
# dataset = MOT17Sequence(seq_name="MOT17-01")
# breakpoint()